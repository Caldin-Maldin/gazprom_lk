...
async def async_setup_entry(
    ...
    sensors = [
        GazpromLKSensor(coordinator, "balance_all", "Общая задолженность", "mdi:currency-rub", "RUB", 2),  # 1 знак после запятой
        GazpromLKSensor(coordinator, "balance_gas", "Задолженность за газ", "mdi:currency-rub", "RUB", 2),  # 1 знак после запятой
        GazpromLKSensor(coordinator, "counter_value", "Показания счетчика", "mdi:counter", "m³", 1),  # 1 знак после запятой
        GazpromLKSensor(coordinator, "counter_rate", "Расход газа", "mdi:gas-burner", "m³", 1),  # 1 знак после запятой
    ]
...
class GazpromLKSensor(GazpromLKEntity, SensorEntity):
    ...
    def __init__(
        ...
        self._attr_suggested_display_precision = precision  # Устанавливаем точность
        
        if unit == "RUB":
            self._attr_device_class = SensorDeviceClass.MONETARY
        # Убрали автоматическое установление precision, теперь оно передается параметром
